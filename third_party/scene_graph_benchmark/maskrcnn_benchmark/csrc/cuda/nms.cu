...
at::Tensor nms_cuda(const at::Tensor boxes, float nms_overlap_thresh) {
  ...
  // Use CUDA caching allocator via at::Tensor (PyTorch 2.x: THCudaMalloc/Free removed)
  auto mask_tensor = at::empty(
      {static_cast<int64_t>(boxes_num) * col_blocks},
      boxes.options().dtype(at::kLong).device(at::kCUDA));
  ...
  // mask_tensor is freed automatically when out of scope.
