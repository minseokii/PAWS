...
def init_teacher_model(cfg_file, AG_dataset_train, gpu_device):
    ...
    print(cfg_file)
    print(model_conf)
    if model_conf.transformer_mode == 'dsgdetr':
        t_model = DSGDETR(mode=model_conf.mode,
                        ...
                        conf=model_conf).to(device=gpu_device)
    else:
        t_model = STTran(mode=model_conf.mode,
                        ...
                        is_wks=model_conf.is_wks,
                        feat_dim=model_conf.feat_dim,
                        obj_dim=model_conf.obj_dim).to(device=gpu_device)
