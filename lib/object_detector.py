...
class detector(nn.Module):
    ...
    def __init__(self, train, object_classes, use_SUPPLY, conf):
        ...
        self.ROI_Align = None
        self.RCNN_Head = None
        self.match_mode = conf.match
        self.conf = conf
        self.det_path = conf.det_path
        self.feat_path = conf.feat_path
    def wk_forward(self, im_data, im_info, gt_boxes, num_boxes, gt_annotation, frame_names, faset_rcnn_model, transforms):
        dets_list, feat_list, base_feat_list = load_feature(frame_names, self.union_box_feature, det_path=self.det_path, feat_path=self.feat_path, is_train=self.is_train, load_feat=True)
        ...
        assign_label_to_proposals_by_dict_for_video(dets_list, feat_list, self.is_train, gt_annotation, pseudo_way=self.pseudo_way, match_mode=self.match_mode, conf=self.conf)

        entry = convert_data(self.is_train, base_feat_list, video_people_det, video_people_feat, video_object_det, \
                 video_object_feat, gt_annotation, frame_names, faset_rcnn_model, transforms, union_box_feature=self.union_box_feature, conf=self.conf)
        return entry
