...
if __name__ == "__main__":
    ...
    for vid in tqdm(sorted(os.listdir(data_root))):
        ...
        for frame in sorted(os.listdir(vid_folder)):
            ...
            # obtain roi feature (reuse base_feat to avoid redundant backbone pass)
            base_feat = model.backbone(to_image_list(img_input).to(model.device).tensors)
            roi_feat = extract_feature_given_bbox_base_feat_torch(
                model, transforms, cv2_img, rect, base_feat, is_mean=True)
            roi_feat = roi_feat.cpu().numpy()
