...
class STTran(nn.Module):
    ...
    def __init__(self, mode='sgdet',
                 ...
                 enc_layer_num=None, dec_layer_num=None, transformer_mode=None, is_wks=True, feat_dim=2048, obj_dim=192, motifs_path=None, conf=None):
        ...
        self.object_classifier = ObjectClassifier(mode=self.mode, obj_classes=self.obj_classes, is_wks=self.is_wks, obj_dim=obj_dim)
        ...
        if self.transformer_mode == 'org':
            ...
            self.glocal_transformer = transformer_wk(enc_layer_num=enc_layer_num, dec_layer_num=dec_layer_num, embed_dim=1936, nhead=8,
                                              dim_feedforward=2048, dropout=0.1, mode='latter', pam=pam)
    ...
    def forward(self, entry):
        ...
        pair_emb = self.pair_mlp(rel_features)
        # Spatial-Temporal Transformer
        global_output, global_attention_weights, local_attention_weights, pair_affinity_emb, all_pair_mask_logits = self.glocal_transformer(features=rel_features, im_idx=entry['im_idx'], pair_emb=pair_emb)
        ...
        if self.motifs_path is None:
            ...
            entry["pair_affinity"] = self.r_compress(pair_affinity_emb)
            entry["rel_emb"] = pair_affinity_emb
        else:
            ...
            entry["pair_affinity"] = self.r_compress(global_output)
            entry["rel_emb"] = pair_affinity_emb
