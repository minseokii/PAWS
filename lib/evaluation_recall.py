...
class BasicSceneGraphEvaluator:
    def __init__(self, mode, AG_object_classes, AG_all_predicates, AG_attention_predicates, AG_spatial_predicates, AG_contacting_predicates,
                 iou_threshold=0.5, constraint=False, semithreshold=None, conf=None):
        self.result_dict = {}
        self.mode = mode
        self.conf = conf
    ...
    def evaluate_scene_graph(self, gt, pred, frame_names=None):
        ...
        for idx, frame_gt in enumerate(gt):
            ...
            # Extract pair_affinity scores if config enabled
            use_pair_affinity = self.conf and getattr(self.conf, 'pa_metric', False)
            if use_pair_affinity and 'pair_affinity' in pred:
                import torch
                pair_affinity_scores = torch.sigmoid(pred['pair_affinity'][pred['im_idx'] == idx]).cpu().numpy().squeeze()
                pair_affinity_scores_all = np.tile(pair_affinity_scores, 3)  # Repeat for att/spa/con
            else:
                pair_affinity_scores_all = None
            ...
            if self.mode == 'predcls':
                pred_entry = {
                    ...
                    'pair_affinity_scores': pair_affinity_scores_all
                }
            else:
                pred_entry = {
                    'pred_boxes': pred['boxes'][:, 1:].cpu().clone().numpy(),
            ...
            frame_name = frame_names[idx] if frame_names is not None else f"frame_{idx}"
            evaluate_from_dict(gt_entry, pred_entry, self.mode, self.result_dict,
                               ...
                               frame_name=frame_name, AG_object_classes=self.AG_object_classes,
                               AG_all_predicates=self.AG_all_predicates)

def evaluate_from_dict(gt_entry, pred_entry, mode, result_dict, method=None, threshold = 0.9,
                       frame_name=None, AG_object_classes=None, AG_all_predicates=None, **kwargs):
    """
    ...
    if method == 'semi':
        ...
        # Multiply pair_affinity scores if available
        if pred_entry.get('pair_affinity_scores') is not None:
            pair_affinity = pred_entry['pair_affinity_scores'][:, None]  # [N_pairs, 1]
            overall_scores = overall_scores * pair_affinity
        ...
        if pred_entry.get('pair_affinity_scores') is not None:
            predicate_scores = predicate_scores * pred_entry['pair_affinity_scores'][score_inds[:, 0]]
        ...
        # Multiply pair_affinity scores if available
        if pred_entry.get('pair_affinity_scores') is not None:
            predicate_scores = predicate_scores * pred_entry['pair_affinity_scores']
    ...
    pred_to_gt, pred_5ples, rel_scores = evaluate_recall(
                ...
                AG_object_classes=AG_object_classes,
                AG_all_predicates=AG_all_predicates,
                **kwargs)
...
def evaluate_recall(gt_rels, gt_boxes, gt_classes,
                    ...
                    iou_thresh=0.5, phrdet=False, AG_object_classes=None, AG_all_predicates=None):
    """
...
def _compute_pred_matches(gt_triplets, pred_triplets,
                 ...
                 AG_object_classes=None, AG_all_predicates=None):
    """
    ...
    for gt_ind, gt_box, keep_inds in zip(np.where(gt_has_match)[0],
        ...
        if phrdet:
            ...
            union_ious = bbox_overlaps(gt_box_union[None], box_union)[0]
            inds = union_ious >= iou_thresh
