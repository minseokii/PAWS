...
|           |     | -- weak/
|           |           | -- gt_pla_gdino_modelfree_05_skip2.pkl            # RAM pseudo-label (PAWS stage-2 input)
|           |           | -- gt_annotation_thres02_keep1.pkl                # PLA stage-1 seed (only if re-generating the RAM pkl)
|           | -- PLA_det_ag_class/                             # VinVL per-frame {dets.npy, feat.npy}
|           | -- PLA_gdino_ag_class/                           # GDino-scored detections (only if re-generating the RAM pkl)
| -- models
...
> **PAWS stage-2 training only needs the RAM pickle** (`gt_pla_gdino_modelfree_05_skip2.pkl`). The PLA stage-1 pickle (`gt_annotation_thres02_keep1.pkl`) is the *seed* that `scripts/generate_pseudo_labels.py` propagates into the RAM pickle — you only need it if you re-run RAM yourself instead of downloading the pre-built pkl. The PLA stage-2 pickle is not used anywhere in PAWS.
...
| `data/action-genome/annotations/weak/gt_pla_gdino_modelfree_05_skip2.pkl` | PAWS RAM pseudo-label — **the only weak annotation PAWS training actually needs**. Download [link TBD], or regenerate via `scripts/generate_pseudo_labels.py`. |
| `data/action-genome/annotations/weak/gt_annotation_thres02_keep1.pkl` *(optional)* | PLA stage-1 weak annotation. Only needed as the seed if you regenerate the RAM pkl yourself; original PLA download links are expired so we re-host [link TBD]. |
| `data/action-genome/PLA_gdino_ag_class/` *(optional)* | GDino-scored detections; only needed if you regenerate the RAM pkl yourself. |
