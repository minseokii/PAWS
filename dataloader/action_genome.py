...
class AG(Dataset):
    ...
    filter_nonperson_box_frame=True, filter_small_box=False, need_relation=True, output_log=False):
        ...
        if ws_object_bbox_path is None:
            print('loading ' + 'annotations/gt_annotation_thres02_keep1.pkl')
            with open(os.path.join(root_path, 'annotations/weak/gt_annotation_thres02_keep1.pkl'), 'rb') as f:
        ...
        for frame_name in person_object_bbox_relationship.keys():
            person_info = person_object_bbox_relationship[frame_name]['person_info']
            # list를 numpy array로 변환 (pkl 호환성을 위해)
            if 'bbox' in person_info and isinstance(person_info['bbox'], list):
                person_info['bbox'] = np.array(person_info['bbox'])
            person_bbox[frame_name] = person_info
    ...
    def get_annotation_by_video_name(self, video_name, use_real_gt=False):
        """
        Get annotation for a specific video by its name

        Args:
            video_name: str, video name (e.g., '001YG.mp4')
            use_real_gt: bool, if True return real_gt_annotations, else return gt_annotations

        Returns:
            annotation list for the video, or None if not found
        """
        for idx, video_frames in enumerate(self.video_list):
            # video_frames is a list like ['001YG.mp4/000089.png', '001YG.mp4/000090.png', ...]
            if len(video_frames) > 0:
                # Extract video name from first frame
                first_frame = video_frames[0]
                current_video_name = first_frame.split('/')[0]

                if current_video_name == video_name:
                    if use_real_gt:
                        return self.real_gt_annotations[idx]
                    else:
                        return self.gt_annotations[idx]
