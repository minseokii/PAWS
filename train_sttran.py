                ...
                if len(pa_loss_list) > 0 and negative_mask.sum() > 0 and use_for_loss.sum() > 0 and "pair_affinity" in pred:
                    ...
                    for pa_mode in pa_loss_list:
                        ...
                        if pa_mode == 'bce':
                            ...
                            bce_weight = getattr(conf, 'pa_weight', 1.0)
                            gamma = getattr(conf, 'focal_gamma', 2.0)
                            # Positive group: focal-weighted BCE
                            pos_logits = logits[use_for_loss]
                            pos_p = torch.sigmoid(pos_logits)
                            pos_focal_w = (1.0 - pos_p) ** gamma
                            pos_bce = (pos_focal_w * F.binary_cross_entropy_with_logits(
                                pos_logits, torch.ones_like(pos_logits), reduction='none')).mean()
                            # Negative group: focal-weighted BCE
                            neg_logits = logits[negative_mask]
                            neg_p = torch.sigmoid(neg_logits)
                            neg_focal_w = neg_p ** gamma  # p(neg correct) = 1-sigmoid, so (1-(1-p))^γ = p^γ
                            neg_bce = (neg_focal_w * F.binary_cross_entropy_with_logits(
                                neg_logits, torch.zeros_like(neg_logits), reduction='none')).mean()
                            losses["pa_focal_loss"] = bce_weight * (pos_bce + neg_bce) / 2

                        elif pa_mode == 'hnm':
                            bce_weight = getattr(conf, 'pa_weight', 1.0)
                            neg_ratio = getattr(conf, 'hnm_neg_ratio', 3)
                            # Positive group: standard BCE
                            pos_bce = F.binary_cross_entropy_with_logits(
                                logits[use_for_loss], torch.ones(use_for_loss.sum(), device=logits.device))
                            # Negative group: select top-k hardest (highest logit = most confident wrong)
                            neg_losses_all = F.binary_cross_entropy_with_logits(
                                logits[negative_mask], torch.zeros(negative_mask.sum(), device=logits.device), reduction='none')
                            num_hard = min(int(use_for_loss.sum().item()) * neg_ratio, int(negative_mask.sum().item()))
                            if num_hard > 0:
                                hard_neg_losses, _ = neg_losses_all.topk(num_hard)
                                neg_bce = hard_neg_losses.mean()
                            else:
                                neg_bce = neg_losses_all.mean()
                            losses["pa_hnm_loss"] = bce_weight * (pos_bce + neg_bce) / 2
                        ...
                        elif pa_mode == 'focal_dist' and conf.teacher_mode_cfg is not None and "pair_affinity" in t_pred:
                            alpha_power = getattr(conf, 'pa_alpha_power', 3.0)
                            bce_weight = getattr(conf, 'pa_weight', 1.0)
                            gamma = getattr(conf, 'focal_gamma', 2.0)
                            ...
                            # Soft-target focal weight: p_t = p*target + (1-p)*(1-target)
                            p = torch.sigmoid(logits)
                            p_t = p * target_pa + (1.0 - p) * (1.0 - target_pa)
                            focal_w = (1.0 - p_t) ** gamma

                            pos_target_mask = target_pa >= 0.5
                            neg_target_mask = target_pa < 0.5
                            if pos_target_mask.sum() > 0 and neg_target_mask.sum() > 0:
                                bce_per = F.binary_cross_entropy_with_logits(logits, target_pa, reduction='none')
                                pos_loss = (focal_w[pos_target_mask] * bce_per[pos_target_mask]).mean()
                                neg_loss = (focal_w[neg_target_mask] * bce_per[neg_target_mask]).mean()
                                losses["pa_focal_dist_loss"] = bce_weight * (pos_loss + neg_loss) / 2

                        elif pa_mode == 'hnm_dist' and conf.teacher_mode_cfg is not None and "pair_affinity" in t_pred:
                            alpha_power = getattr(conf, 'pa_alpha_power', 3.0)
                            bce_weight = getattr(conf, 'pa_weight', 1.0)
                            neg_ratio = getattr(conf, 'hnm_neg_ratio', 3)
                            ...
                            neg_target_mask = target_pa < 0.5
                            if pos_target_mask.sum() > 0 and neg_target_mask.sum() > 0:
                                # Pos group: standard BCE on soft target
                                pos_loss = F.binary_cross_entropy_with_logits(
                                    logits[pos_target_mask], target_pa[pos_target_mask])
                                # Neg group: top-k hardest by per-element BCE
                                neg_losses_all = F.binary_cross_entropy_with_logits(
                                    logits[neg_target_mask], target_pa[neg_target_mask], reduction='none')
                                num_hard = min(int(pos_target_mask.sum().item()) * neg_ratio,
                                               int(neg_target_mask.sum().item()))
                                if num_hard > 0:
                                    hard_neg_losses, _ = neg_losses_all.topk(num_hard)
                                    neg_loss = hard_neg_losses.mean()
                                else:
                                    neg_loss = neg_losses_all.mean()
                                losses["pa_hnm_dist_loss"] = bce_weight * (pos_loss + neg_loss) / 2
