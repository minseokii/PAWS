...
wandb.init(
    ...
    tags=['dsgdetr', 'stage2' if conf.teacher_mode_cfg else 'stage1', conf.mode, conf.match],  # Tags for filtering
)
            ...
            if entry != None:
                ...
                if negative_mask.sum() > 0 and use_for_loss.sum() > 0 and "all_pair_mask_logits" in pred:
                    ...
                    if len(all_pair_mask_logits) > 0:
                        ...
                        # Build masks using sequences (DSG-DETR: temporal grouping by object class)
                        sequences = pred.get('sequences', None)
                        ...
                        if sequences is not None:
                            for seq_idx, seq_indices in enumerate(sequences):
                                num_pairs = len(seq_indices)
                                positive_mask[seq_idx, :num_pairs] = positive_mask_flat[seq_indices]
                                gt_mask[seq_idx, :num_pairs] = gt_rel[seq_indices]
                                valid_mask[seq_idx, :num_pairs] = True
                                if conf.teacher_mode_cfg is not None:
                                    target_logit_padded[seq_idx, :num_pairs] = target_logit[seq_indices]
